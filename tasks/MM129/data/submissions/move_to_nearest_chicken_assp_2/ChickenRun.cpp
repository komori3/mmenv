...
struct ASSP {
    ...
    ASSP(const std::vector<std::vector<int>>& distance_matrix) :
        V(distance_matrix.size()), dist(distance_matrix), next(V, std::vector<int>(V)) {
        build();
    }
    void build() {
...
struct State {
    ...
    pii calc_next_cell(int r1, int c1, int r2, int c2) const {
        // (r1, c1) -> (r2, c2) ÇÃç≈íZòHÇ…Ç®ÇØÇÈ (r1, c1) ÇÃéüÇÃÉZÉã
    ...
    std::vector<Move> move_to_nearest_chicken_assp() {
        ...
        for (const auto& e : persons) {
            ...
            if (tr == -1) continue;
            int r2, c2;
            std::tie(r2, c2) = calc_next_cell(pr, pc, tr, tc);
            if (grid[r2][c2] == 'P') {
                for (int d = 0; d < 4; d++) {
                    int nr = pr + dr[d], nc = pc + dc[d];
                    if (!is_inside(nr, nc) || grid[nr][nc] == '#' || grid[nr][nc] == 'P' || used[nr][nc]) continue;
                    grid[pr][pc] = '.';
                    if (grid[nr][nc] == CHICKEN) score += N * N - (turn - 1);
                    grid[nr][nc] = 'P';
                    used[nr][nc] = true;
                    moves.emplace_back(pr, pc, nr, nc);
                    break;
                }
                continue;
            }
...
..P#..P#..
#.C...#...)";
    std::istringstream iss(sboard);
    string line;
    vector<string> grid;
    while (iss >> line) grid.push_back(line);

    dump(sboard);
    dump(grid);

    int N = grid.size(), V = N * N;
    assert(grid.size() == N);
    for (const auto& s : grid) assert(s.size() == N);
    dump(grid);
    for (auto& s : grid) for (auto& c : s) if (c != '.' && c != '#') c = '.';
    dump(grid);
    int inf = INT_MAX / 8;
    vector<vector<int>> d(V, vector<int>(V, inf)); // (rc1, rc2)
    ...
    auto get_path = [&](int r1, int c1, int r2, int c2) {
        int u = r1 * N + c1, v = r2 * N + c2;
        vector<pii> ret;
        for (int cur = u; cur != v; cur = next[cur][v]) {
            assert(grid[cur / N][cur % N] != '#');
            ret.emplace_back(cur / N, cur % N);
        }
        ret.emplace_back(v / N, v % N);
        assert(grid[v / N][v % N] != '#');
        return ret;
    };

    dump(get_path(0, 1, 9, 9));

    {
        auto p = assp.get_path(0 * N + 1, 9 * N + 9);
        vector<pii> ps;
        for (const int u : p) ps.emplace_back(u / N, u % N);
        cerr << ps << endl;
    }

    for (const auto& v : grid) cerr << v << endl;
}
...
int main() {
    ...
    if (local) {
        TestCasePtr tc = std::make_shared<TestCase>(2, verbose);
