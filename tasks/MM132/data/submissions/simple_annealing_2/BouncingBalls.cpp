...
struct State {
    ...
    void move(MoveGunPtr move) {
        m_gr[move->id] = move->r2;
        m_gc[move->id] = move->c2;
    }
...
int main() {
...
#if 0
    std::ifstream ifs("C:\\Users\\komori3\\OneDrive\\dev\\heuristic\\tasks\\MM132\\in\\58.in");
    std::istream& in = ifs;
    std::ofstream ofs("C:\\Users\\komori3\\OneDrive\\dev\\heuristic\\tasks\\MM132\\out\\58.out");
    std::ostream& out = ofs;
#else
    ...
    while ((now_time = timer.elapsed_ms()) < end_time) {
        ...
        double temp = get_temp(100.0, 0.0, (now_time - start_time) / (end_time - start_time));
